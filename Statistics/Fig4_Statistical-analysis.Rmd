...
Null hypothesis : "Metal quotas (Me:C medians) of bacteria and phytoplankton are equal"
...
## 2 Subsetting
Note, there is an crazy outlier in the phytoplakton data @ Cu:C of 30, this is not included in the test

```{r message=FALSE}
data_no_cyano <- data%>% filter(Cu_C<20, Domain %in% c("Euk_algae","Het_bacteria"))
...
Variances are NOT equal as p= 0.027
...
#####Wilcox Rank sum test (Mann-Whitney U test) and Moods median test (for further exploration). Result: since p-value for both Wilcox and Moods median is >0.05, we reject accept the null hypothesis that Cu quotas of bacteria and phytoplankton on this dataset are the same (significantly not different)
