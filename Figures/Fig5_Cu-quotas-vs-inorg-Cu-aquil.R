...
phytos<-dat%>%
	filter(Strategy == "Photosynthetic", Domain == "Euk_algae",Cu_C<13)%>%
	ggplot(aes(x = Cu_prime_log,y = Cu_C,fill = Taxon))+
  geom_jitter(shape = 21, size=6.5,alpha=1/2, width=0.02)+
	annotate("rect", xmin = 14.5, xmax = 12, ymin = 2.8, ymax = 4.9, alpha =.3, fill ="gray")+
	annotate("segment", x = 13.74, xend = 12, y = 6, yend = 6,
					 arrow=arrow(ends="both", angle=90,length=unit(.2,"cm")))+
	annotate("text", x = 12.8, y = 7.0, label="log [Cu'] range in seawater", size=5.5) +
	annotate("text", x = 13.6, y = 12.5, label="Eukaryotic Phytoplankton", fontface = 2,size=5.5) +
...
bact<-dat%>%
	...
	ggplot(aes(x=Cu_prime_log,y=Cu_C,fill=Taxon))+
	geom_jitter(shape=23, size=6.5,alpha=1/2)+
	annotate("rect", xmin=14.5, xmax=12, ymin=2.8, ymax=4.9, alpha=.3, fill="gray")+
	annotate("segment", x = 13.74, xend = 12, y = 6, yend = 6,
					 arrow=arrow(ends="both", angle=90,length=unit(.2,"cm")))+
	annotate("text", x = 12.8, y = 7.0, label="log [Cu'] range in seawater", size=5.5) +
	annotate("text", x = 13.7, y = 12.5, label="Heterotrophic bacteria", fontface = 2,size=5.5) +
	scale_x_reverse(name="-log ([Cu'])",expand=c(0,0))+
	ylab(expression("Cu:C" ~(mu~mol:mol)))+
	scale_y_continuous(breaks = c(0,1.5,3,4.5,6,7.5,9,10.5,12),limits = c(0,12.6))+
...
plot <-plot_grid(phytos,bact,
								 labels = c("A", "B"),label_size = 16, align = "h")

#save_plot(filename = "Fig5_bact.png", plot = bact, base_height = 4, base_width = 9.5 )

save_plot (filename="Fig5_Cu-quotas-vs-inorg-Cu-published.tiff", 
           plot= plot, base_height= 6.5, base_width =7.5)
